...
class RAGSystem:
    ...
    def get_querry_engine(self,top_k=2):
        if not self.index:
            raise ValueError('No index available . Please Process the PDf first')
    ...
    def generate_response(self,query_engine,query):
        try :
            ...
            if not context.strip():
                return 'No relevant information from the PDF document'
            ...
            return response_text if response_text else "unable to generate a response"
        
        
        except Exception as e:
            print(f'Error generating a  response:{str(e)}')
            return f'Error proceesing your question {str(e)}'
